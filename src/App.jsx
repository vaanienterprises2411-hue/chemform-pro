...
const CATEGORIES = [
  ...
  { id:"sizing",    label:"Sizing",      icon:"🧵", color:"#84cc16" },
  { id:"pharma",    label:"Pharma API",  icon:"💊", color:"#e879f9" },
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        headers:{
          ...
          "x-api-key":"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",
...
function RequestFormula({user, planKey, currency, onUpgrade}){
  ...
  const handleSubmit=()=>{
    ...
    if(isPaid){ setPayStep("processing"); return; } // Pro/Enterprise: skip payment
    setPayStep("payment"); // Others: show payment
  };
...
export default function App(){
  ...
  return(
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"#060b14",height:"100vh",color:"#f1f5f9",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* Category tabs */}
      <div style={{borderBottom:"1px solid #1e293b",background:"#060b14",flexShrink:0,overflowX:"auto",display:"flex",WebkitOverflowScrolling:"touch"}}>
        {CATEGORIES.map(cat=>(
          <button key={cat.id} onClick={()=>{setCatId(cat.id);setSelected(null);setSearch("");}}
            style={{padding:"9px 12px",border:"none",background:catId===cat.id?cat.color+"18":"transparent",
              cursor:"pointer",color:catId===cat.id?cat.color:"#475569",
              borderBottom:`2px solid ${catId===cat.id?cat.color:"transparent"}`,
              fontWeight:catId===cat.id?700:500,fontSize:10,transition:"all 0.2s",
              marginBottom:-1,whiteSpace:"nowrap",flexShrink:0,
              display:"flex",alignItems:"center",gap:4}}>
            {cat.icon} {cat.label}
            {cat.enterprise&&<span style={{background:"#fb923c22",color:"#fb923c",fontSize:7,fontWeight:800,padding:"1px 5px",borderRadius:3}}>ENT</span>}
            {cat.special&&<span style={{background:"#f59e0b22",color:"#f59e0b",fontSize:7,fontWeight:800,padding:"1px 5px",borderRadius:3}}>NEW</span>}
            {priority.includes(cat.id)&&!cat.enterprise&&!cat.special&&<span style={{background:"#34d39922",color:"#34d399",fontSize:7,fontWeight:800,padding:"1px 5px",borderRadius:3}}>★</span>}
          </button>
        ))}
      </div>
