...
export default function App(){
  ...
  return(
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"#060b14",height:"100vh",color:"#f1f5f9",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* Category tabs */}
      <div style={{borderBottom:"1px solid #1e293b",background:"#060b14",flexShrink:0}}>
        <div style={{display:"flex",overflowX:"auto",borderBottom:"1px solid #0f172a"}}>
          {CATEGORIES.filter(c=>!c.enterprise&&!c.special&&!c.paid).map(c=>(
            <button key={c.id} onClick={()=>{setCatId(c.id);setSelected(null);setSearch("");}} style={{padding:"7px 11px",border:"none",background:"none",cursor:"pointer",color:catId===c.id?c.color:"#475569",borderBottom:`2px solid ${catId===c.id?c.color:"transparent"}`,fontWeight:catId===c.id?700:500,fontSize:10,transition:"all 0.2s",marginBottom:-1,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:3}}>
              {c.icon} {c.label}
              {priority.includes(c.id)&&<span style={{background:"#34d39922",color:"#34d399",fontSize:7,fontWeight:800,padding:"1px 4px",borderRadius:3}}>★</span>}
            </button>
          ))}
        </div>
        <div style={{display:"flex",background:"#030710",padding:"2px 0"}}>
          {CATEGORIES.filter(c=>c.enterprise||c.special||c.paid).map(c=>(
            <button key={c.id} onClick={()=>{setCatId(c.id);setSelected(null);setSearch("");}} style={{padding:"5px 11px",border:"none",background:catId===c.id?c.color+"18":"transparent",cursor:"pointer",color:catId===c.id?c.color:"#475569",borderBottom:`2px solid ${catId===c.id?c.color:"transparent"}`,fontWeight:catId===c.id?700:500,fontSize:10,transition:"all 0.2s",marginBottom:-1,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4}}>
              {c.icon} {c.label}
