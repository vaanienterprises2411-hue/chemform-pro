...
import { sendOTP, verifyOTP, getProfile, upsertProfile, getUsage, incrementUsage, saveFormulaRequest, updatePlan, setSession, getSession, clearSession } from "./supabase.js";
...
function LoginScreen({onLogin}){
  ...
  const sendOtp=async()=>{
    ...
    setLoading(false);
    if(ok){setStep("otp");setErr("OTP sent! Check your email/SMS.");}
    else setErr("Could not send OTP. Check contact and try again.");
  };
...
export default function App(){
  ...
  const useQuota=async(type)=>{
    ...
    if(user?.id && getSession()) {
      await incrementUsage(user.id, type, getSession());
    }
  };
  ...
  return(
    ...
    if(!name.trim()||!industry){setErr("Please fill name and select industry");return;}
    setLoading(true);
    const profile={id:userId,name,contact:contact.trim(),city,country,industry,plan:"free"};
    await upsertProfile(profile,userSession);
    setLoading(false);
    onLogin({...profile,session:userSession});
